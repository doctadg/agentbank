/**
 * Hyperliquid SDK Client Wrapper (AgentBank Extract)
 *
 ...
 *
 * Extracted from PerpsTrader with message-bus/snapshot-service removed.
 * Replaced with eventBus for internal event publishing.
 */
...
export class HyperliquidClient {
    ...
    constructor() {
        ...
        // HYPERLIQUID_AGENT_KEY: preferred — a Hyperliquid "API wallet" that can
        // place/cancel orders but cannot withdraw. The funding key (mainAddress)
        // stays cold. Approve via Hyperliquid UI: Account → API Wallets.
        const privateKey = process.env.HYPERLIQUID_AGENT_KEY
    ...
    async placeOrder(params: {
        ...
        const maxRetries = 1; // Allow 1 retry for transient network errors
